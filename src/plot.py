...
def mk_review_index_plot(args, obj):
    if "reviewed_patches" not in obj["metrics"] or \
       "total_patches" not in obj["metrics"]:
    ...
    plt.title(f"Ratio of reviews over total patches: {obj['repo']} repo {timeframe}", fontsize=BASE_FONTSIZE*1.2)
...
def ordinal(num):
    ...
    elif num % 10 == 2: suffix = "nd"
    elif num % 10 == 3: suffix = "rd"
...
def mkplots(args, obj, pretty_headers):
    ...
    if "internal_patches_to_org_files" not in obj["metrics"] or \
       "total_patches" not in obj["metrics"]:
    ...
    for header_id, header in enumerate(obj["metrics"]):
        ...
        for y_id, org in enumerate(obj["orgs"]):
            ...
            if org in args.highlight:
                plt.plot(xaxis, yaxis, '-o', color='orange', linewidth=14, alpha=0.4, label=org)
            else:
                plt.plot(xaxis, yaxis, '-o', label=org)

        plt.title(f"{pretty_headers[header_id]}: {obj['repo']} repo {timeframe}", fontsize=BASE_FONTSIZE*1.2)
        plt.xlabel("Time")
        ...
        plt.legend([handles[i] for i in order],
                   ...
                   fontsize=BASE_FONTSIZE, loc="upper left")
